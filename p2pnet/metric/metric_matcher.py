...
class HungarianMatcher_Crowd_metric_ver(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_point: float = 1, matcher_cal="gpu"):
        """Creates the matcher
        ...
        self.matcher_cal = matcher_cal
        if not (matcher_cal == "gpu" or matcher_cal == "cpu"):
            raise ValueError("matcher_cal must be gpu or cpu")
    ...
    def forward(self, outputs, targets):
        ...
        if sum([len(v["point"]) for v in targets]) < 5000:
            # if self.matcher_cal == "gpu":
            ...
            # Final cost matrix
            # self.cost_point,self.cost_classは係数
            ...
            outputs_cpu = {}
            outputs_cpu["pred_logits"] = outputs["pred_logits"].detach().cpu()
            outputs_cpu["pred_points"] = outputs["pred_points"].detach().cpu()

            bs, num_queries = outputs_cpu["pred_logits"].shape[:2]

            # We flatten to compute the cost matrices in a batch
            out_prob = (
                outputs_cpu["pred_logits"].flatten(0, 1).softmax(-1)
            )  # [batch_size * num_queries, num_classes]
            out_points = outputs_cpu["pred_points"].flatten(
                0, 1
            )  # [batch_size * num_queries, 2]

            # Also concat the target labels and points
            # tgt_ids = torch.cat([v["labels"] for v in targets])
            tgt_ids = torch.cat([v["labels"].detach().cpu() for v in targets])
            tgt_points = torch.cat([v["point"].detach().cpu() for v in targets])
            ...
            sizes = [len(v["point"].detach().cpu()) for v in targets]
            indices = [
                linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))
            ]
...
def build_matcher_crowd(cfg):
    # return HungarianMatcher_Crowd_metric_ver(
