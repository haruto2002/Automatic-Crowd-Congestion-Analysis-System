...
from prediction.dataset import create_dataloader
...
def evaluate(cfg, model, data_loader, device, global_rank, trans_data):
    ...
    for i, batch in enumerate(data_loader):
        ...
        for img, info, points, scores in zip(
            ...
            points = reconstruction(np_img, points, trans_data)
            save_detection(cfg.detection_save_dir, points, scores, info)
        if cfg.default.bar and global_rank == 0:
            bar.update(1)
    if cfg.default.bar and global_rank == 0:
        bar.close()
...
def save_detection(save_dir, points, scores, info):
    ...
    plot_save_dir = f"{patch_save_dir}/plot"
...
def input_info(cfg):
    # img_dir = "tracking_results/WorldPorters_noon/img"
    img_dir = cfg.img_dir
    img_size = (4320, 7680)
    ...
    trans_data = None if (height == new_height and width == new_width) else (height, width, new_height, new_width)
...
if __name__ == "__main__":
    ...
    save_dir = f"{cfg.out_dir}/detection"
