...
from network.backbone import Backbone_VGG
from network.loss_fn import SetCriterion_Crowd
from network.matcher import HungarianMatcher_Crowd
from network.p2pnet import P2PNet
...
def suggest_network(cfg, device):
    ...
    model = P2PNet(backbone, cfg.network.row, cfg.network.line, device)
    if cfg.default.finetune:
        state_dict = torch.load(cfg.network.init_weight)
