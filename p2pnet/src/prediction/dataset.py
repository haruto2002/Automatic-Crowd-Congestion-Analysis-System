...
class DangerDataset(Dataset):
    ...
    def __init__(self, img_dir, img_size=(4320, 7680), separate_size=(1152, 1280), padded_size=(4608, 7680)):
        ...
        # 画像サイズの確認
        assert cv2.imread(self.path2img_list[0]).shape[:2] == img_size, "img size is wrong"
        ...
        self.trans_data = None if (height == new_height and width == new_width) else (height, width, new_height, new_width)

    def __len__(self):
        return self.total_patches

    def __getitem__(self, index):
        # インデックスから元画像とパッチの位置を特定
        ...
        # 情報の作成
        info = {
            "img_name": os.path.splitext(os.path.basename(self.path2img_list[img_idx]))[0],
    ...
    def _padding_img(self, img):
        ...
        padded_img = np.zeros((self.padded_size[0], self.padded_size[1], 3), dtype=img.dtype)
