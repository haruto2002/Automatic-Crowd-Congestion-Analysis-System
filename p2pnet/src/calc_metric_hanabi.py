...
from metric.utils_predict_hanabi import load_from_path, predict_slide, predict


def calc_loc_metrics(dist_matrix, gt_num, sigma):
...
def vis_point(save_dir, path2img, gts, detections, indices, sigma):
    if indices is None:
        return
    tp_gt_index, fn_gt_index, tp_pred_index, fp_pred_index = indices
    # color_list = [(0, 255, 0), (51, 81, 0), (0, 0, 255), (255, 0, 0)]
    color_list = [(0, 255, 0), (255, 0, 255), (0, 0, 255), (0, 255, 255)]
...
def vis_point3(save_dir, path2img, gts, detections, indices, sigma):
    if indices is None:
        return
    ...
    img = cv2.imread(path2img)
    img_copy = img.copy()
    for i, idx_list in enumerate(indices[2:]):
        # 0:FN(gt) >> 1:TP(pred) >> 2:FP(pred)
        for idx in idx_list:
            p = detections[idx]["coordinate"]
            if i == 0:  # TP
...
def vis_point_sotsuron(save_dir, path2img, gts, detections, indices, sigma):
    # if indices is None:
    # add code
    ...
    cv2.imwrite(
        save_dir
        + f"/sigma{sigma}_TP{len(tp_gt_index)}_FN{len(fn_gt_index)}_FP{len(fp_pred_index)}_GT.png",
        img_copy1,
    )
    # Pred
    img_copy2 = img.copy()
    ...
    cv2.imwrite(
        save_dir
        + f"/sigma{sigma}_TP{len(tp_gt_index)}_FN{len(fn_gt_index)}_FP{len(fp_pred_index)}_Pred.png",
        img_copy2,
    )


def vis_point_sotsuron_v2(save_dir, path2img, gts, detections, indices, sigma):
    if indices is None:
        return
...
def vis_point_sotsuron_v3(save_dir, path2img, gts, detections, indices, sigma):
    if indices is None:
        return
    ...
    cv2.imwrite(
        save_dir
        + f"/sigma{sigma}_TP{len(tp_gt_index)}_FN{len(fn_gt_index)}_FP{len(fp_pred_index)}_error.png",
        img_copy,
    )
...
def calc_metric(
    ...
    for i, data_dir in enumerate(tqdm(data_dir_list)):
        ...
        if img.shape == (4320, 7680, 3) or img.shape == (4320, 3840, 3):
            ...
            # h_size = 2160
            # w_size = 3840
            detections, gts = predict_slide(
                img_id, img, gt_points, model, device, h_size=h_size, w_size=w_size
            )
        else:
            detections, gts = predict(img_id, img, gt_points, model, device)

        gt_num = len(gts)
        dist_matrix = calc_dist(detections, gts)
        tp, fp, fn, indices = calc_loc_metrics(dist_matrix, gt_num, sigma)
        ...
        if each_save:
            ...
            # vis_point(each_save_dir, path2img, gts, detections, indices, sigma)
            # vis_point2(each_save_dir, path2img, gts, detections, indices, sigma)
            # vis_point3(each_save_dir, path2img, gts, detections, indices, sigma)
            # vis_point_sotsuron(each_save_dir, path2img, gts, detections, indices, sigma)
            vis_point_sotsuron_v2(
                each_save_dir, path2img, gts, detections, indices, sigma
            )
            # vis_point_sotsuron_v3(
            #     each_save_dir, path2img, gts, detections, indices, sigma
            # )
            each_saver(
                each_save_dir,
...
def main(
    save_dir, weight_path, weight_name, dataset_name, resource, sigma, each_save, gpu_id
):
    ...
    if (
        ...
        # # custom
        # data_dir_list = []
        # for num in [165, 264, 130]:
        #     data_dir_list.append(root_dir + f"test/scene{num:04d}")
