...
import track_utils.kalman_filter as kalman_filter
...
def euclidean_distance(atracks, btracks):
    ...
    apoints = np.asarray([track.point for track in atracks])
    bpoints = np.asarray([track.point for track in btracks])

    # ユークリッド距離を計算
    cost_matrix = cdist(apoints, bpoints, metric="euclidean")
    return cost_matrix
...
def maha_distance(atracks, btracks, kalman_filter_instance, metric="maha"):
    ...
    :param metric: 距離計算に使用するメトリック ('maha'または'euclidean')
    ...
    if metric == "euclidean":
        apoints = np.asarray([track.point for track in atracks])
        return cdist(apoints, bpoints, metric="euclidean")

    # マハラノビス距離を使用する場合（デフォルト）
    # 各トラックに対してマハラノビス距離を計算
