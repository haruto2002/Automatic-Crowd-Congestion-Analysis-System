...
def get_all_track(track_dir, bev_file, start_frame, end_frame):
    assert os.path.exists(track_dir), f"The track directory {track_dir} does not exist."
    ...
    if bev_file is not None:
        all_track = bev_trans(bev_file, all_track)

    return all_track


def bev_trans(bev_file, all_track):
    assert os.path.exists(bev_file), f"The bev file {bev_file} does not exist."
    matrix = np.loadtxt(bev_file)
