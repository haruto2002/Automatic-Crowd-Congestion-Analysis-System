...
def get_bev_points(bev_dir, crop_area):
    ...
    matrix = np.loadtxt(f"{bev_dir}/matrix.txt")
...
def get_cropped_danger_score(map_danger_score, bev_crop_data, grid_size):
    ...
    # map_danger_score の全画素を対象に Point-in-Polygon 判定
    h, w = map_danger_score.shape
    # Build mesh with horizontal as X, vertical as Y
    xx, yy = np.meshgrid(np.arange(w), np.arange(h))
    coords = np.vstack((xx.ravel(), yy.ravel())).T  # shape=(h*w, 2)
    mask = polygon.contains_points(coords).reshape(h, w)

    # マスクで抽出
    cropped_vals = map_danger_score[mask]  # 1次元配列
...
def get_cropped_density(vec_data, bev_crop_data, real_scale=13.5):
    ...
    # Check if (x,y) of each vector is inside polygon
